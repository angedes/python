...
def decomposeP(est_P):
    ...
    est_K, est_R = np.linalg.qr(est_P[:, :3])     # qr è un tipo di decomposizione che permette di trovare una stima delle matrici K e P
    D = np.diag(np.sign(np.diag(est_K)))      # vado a fare la diagonale del segno della diagonale di K
    if np.linalg.det(D) < 0:                  # questo non bene perchè lo faccia
        D[1,1] *= -1
    K = est_K @ D                            # K diventa il prodotto matriciale della stima di K con D
...
def dlt(p2Ds, p3Ds):
    # la funzione controlla la dimensione degli array
    rows1, cols1 = np.shape(p2Ds)
    rows2, cols2 = np.shape(p3Ds)

    assert (rows1 == rows2)                  #se le righe dei punti 2d e 3d sono uguali allora -> , altrimenti stampa un messaggio di errore
    ...
    est_P = vh[-1]                    #la soluzione è l'ultima colonna di v (0vvero vh) ed è la stima di P (est_P -> estimation of P

    est_P = est_P.reshape(3, 4)       # semplice reshape di est_P

    P, K, T = decomposeP(est_P)
    # funzione che permette di decomporre P (dando in input una stima di P), ricavandone K e T (guarda sopra)
    return P, K, T
