...
def plotLabeledImagePoints(x, labels, strColor='r', offset= (20, -20)):
    """
...
def plotMarkersImagePoints3D(ax, x, **kwargs):
    ax.scatter(x[0, :], x[1, :], x[2, :], **kwargs)


def plotMarkersImagePoints(x, **kwargs):
    plt.scatter(x[0, :], x[1, :], **kwargs)


def plot2Dline(l, color='-g', linewidth=3, im2w=0):
    x = np.array([0, im2w])
    y = -(x * l[0] + l[2]) / l[1]
...
def plotWorldFromC1(T_c2_c1, p3D, title=None):
    """ Plots 3D world from c1 perspective and the set of points"""
    plt.figure()
    ...
    xFakeBoundingBox = np.linspace(0, 6, 4)
    yFakeBoundingBox = np.linspace(0, 6, 4)
    zFakeBoundingBox = np.linspace(0, 6, 4)
    plt.plot(xFakeBoundingBox, yFakeBoundingBox, zFakeBoundingBox, 'w.')

    drawRefSystem(ax, np.eye(4, 4), '-', 'c1')
    drawRefSystem(ax, T_c2_c1, '-', 'C2')
    plotMarkersImagePoints3D(ax, p3D, c="red", marker='+', label="3D points")
    return ax

def plotWorldFromC1_2(T_c3_c1, T_c2_c1, p3D, title=None):
    """ Plots 3D world from c1 perspective and the set of points"""
    plt.figure()
    if title is not None:
        plt.title(title)
    ...
    drawRefSystem(ax, np.eye(4, 4), '-', 'C1')
    drawRefSystem(ax, T_c2_c1, '-', 'C2')
    drawRefSystem(ax, T_c3_c1, '-', 'C3')
    plotMarkersImagePoints3D(ax, p3D, c="red", marker='+', label="3D points")
    return ax

def plotMatchedPoints(matches, keypoints1, keypoints2, pict1, pict2, title):
    ...
    x1_matches = np.empty([3, len_matches])
    x2_matches = np.empty([3, len_matches])

    j = 0

    for i in range(len(matches)):
        if matches[i] != -1:
            x1_matches[0, j] = keypoints1[0, i]
            x1_matches[1, j] = keypoints1[1, i]
            x1_matches[2, j] = 1

            x2_matches[0, j] = keypoints2[0, matches[i]]
            x2_matches[1, j] = keypoints2[1, matches[i]]
            x2_matches[2, j] = 1
    ...
    plt.imshow(pict1)
    ax.set_title('Picture 1')
    plotMarkersImagePoints(x1_matches, color='b', marker='+')
    ax = fig.add_subplot(1, 2, 2)
    plt.imshow(pict2)
    plotMarkersImagePoints(x2_matches, color='r', marker='+')
    ax.set_title('Picture 2')
    plt.show()

    return x1_matches, x2_matches
