...
def get_pose(pts1, pts2, K1, K2, image_shape, img1=None, img2=None):
    print("GET POSE")
    pts1_undistorted = cv2.undistortPoints(pts1, cameraMatrix=K1, distCoeffs=None)
    pts2_undistorted = cv2.undistortPoints(pts2, cameraMatrix=K2, distCoeffs=None)
    # points1u is a (N, 1, 2) float32 from cornerSubPix
    points1u = pts1_undistorted / np.linalg.norm(pts1_undistorted)
    points2u = pts2_undistorted / np.linalg.norm(pts2_undistorted)

    pts1 = points1u.reshape(4904, 2)
    pts2 = points2u.reshape(4904, 2)
    ...
    #ran, pts1, pts2 = ransac(pts1, pts2, K1, K2, image_shape, img1=None, img2=None)
...
def get_correspondances2(img1, img2):
    # Initiate SIFT detector
    print("SIFT")
    sift = cv2.SIFT_create()
    kp1, des1 = sift.detectAndCompute(img1, None)
    ...
    draw_params = dict(matchColor=(0, 255, 0), singlePointColor=(255, 0, 0), matchesMask=matchesMask, flags=0)
    img3 = cv2.drawMatchesKnn(img1, kp1, img2, kp2, matches, None, **draw_params)
    cv2.imwrite("correspondances_all2.png", img3)
    for pts in pts1:
        cv2.circle(img1, (int(pts[1]), int(pts[0])), 5, (0, 255, 0))
    cv2.imwrite("correspondances4.png", img1)
    for pts in pts2:
        cv2.circle(img2, (int(pts[1]), int(pts[0])), 5, (0, 255, 0))
    cv2.imwrite("correspondancesOld.png", img2)
    return pts1, pts2
...
if __name__ == '__main__':
    ...
    # with Superglue img1, img2

    match_data = np.load("img1_img2_matches.npz")
    ...
    pts1, ptsOld = get_correspondances2(img1.copy(), imgOld.copy())

    # with Superglue img1, imgOld

    match_data = np.load("img1_old_matches.npz")
    x1_SPSG = match_data['keypoints0']
    xOld_SPSG = match_data['keypoints1']
    x1_SPSG = np.vstack((x1_SPSG.T, np.ones((1, x1_SPSG.shape[0]))))
    xOld_SPSG = np.vstack((xOld_SPSG.T, np.ones((1, xOld_SPSG.shape[0]))))
    print("Keypoints 0 : ", x1_SPSG)
    print("Keypoints 1 : ", xOld_SPSG)
    matches_SG_1_Old = match_data['matches']
    matches_SG_1_Old_confidence = match_data['match_confidence']
    print("Matches_SG_1_Old : ", matches_SG_1_Old)
    print("Matches_SG_1_Old.shape : ", matches_SG_1_Old.shape)
    img1 = cv2.cvtColor(cv2.imread("img1.png"), cv2.COLOR_BGR2RGB)
    imgOld = cv2.cvtColor(cv2.imread("old.png"), cv2.COLOR_BGR2RGB)
    new1 = cv2.resize(img1, dsize=(640, 480))
    old = cv2.resize(imgOld, dsize=(640, 480))

    len_matches_1_Old = 0
    for i in range(len(matches_SG_1_Old)):
        if matches_SG_1_Old[i] != -1:
            len_matches_1_Old += 1

    x1_matches_SG = np.empty([3, len_matches_1_Old])
    xOld_matches_SG = np.empty([3, len_matches_1_Old])
    j = 0
    print('matches_SG_1_Old.shape=', matches_SG_1_Old.shape)
    # print('len_matches = : ', len_matches)
    for i in range(len(matches_SG_1_Old)):
        if matches_SG_1_Old[i] != -1:
            ...
            x1_matches_SG[1, j] = x1_SPSG[1, i]
            x1_matches_SG[2, j] = 1
            xOld_matches_SG[0, j] = xOld_SPSG[0, matches_SG_1_Old[i]]
            xOld_matches_SG[1, j] = xOld_SPSG[1, matches_SG_1_Old[i]]
            xOld_matches_SG[2, j] = 1

            j += 1

    print('x1_matches_SG: ', x1_matches_SG)
    # print('x1_matches_SG.shape = : ', x1_matches_SG.shape)
    print('xOld_matches_SG: ', xOld_matches_SG)
    # print('x2_matches_SG.shape = : ', x2_matches_SG.shape)
    x1 = (x1_matches_SG[:2, :]).T
    xOld = (xOld_matches_SG[:2, :]).T
    print('x1 = ', x1)
    print('xOld = ', xOld)
    # print('x1.shape = ', x1.shape)
    ...
    ax = fig.add_subplot(1, 2, 2)
    plt.imshow(old)
    pld.plotMarkersImagePoints(xOld_matches_SG, color='r', marker='+')
...
def get_pose(pts1, pts2, K1, K2, image_shape, img1=None, img2=None):
    print("GET POSE")
    #normalize points
    pts1_normalized = cv2.undistortPoints(pts1, K1, None)
    pts2_normalized = cv2.undistortPoints(pts2, K2, None)
    #apply ransac
    E, mask = cv2.findEssentialMat(pts1_normalized, pts2_normalized, cameraMatrix=K1, method=cv2.RANSAC, prob=0.99, threshold=3.0)
    pts1_normalized = pts1_normalized[mask.astype(bool).squeeze()]
    pts2_normalized = pts2_normalized[mask.astype(bool).squeeze()]
    #ran, pts1, pts2 = ransac(pts1, pts2, K1, K2, image_shape, img1=None, img2=None)
    points, R, t, mask = cv2.recoverPose(E, pts1_normalized, pts2_normalized, cameraMatrix=K1)
    M1 = np.hstack((R, t))
    M2 = np.hstack((np.eye(3, 3), np.zeros((3, 1))))
    pts1_normalized = pts1_normalized[mask.astype(bool).squeeze()]
    pts2_normalized = pts2_normalized[mask.astype(bool).squeeze()]
    P1 = np.dot(K1, M1)
    P2 = np.dot(K2, M2)
    points_4d_hom = cv2.triangulatePoints(P1, P2, np.expand_dims(pts1_normalized, axis=1), np.expand_dims(pts2_normalized, axis=1))
    points_4d = points_4d_hom / points_4d_hom[-1:, :]
    points_3d = points_4d[:3, :].T
    points_3d = points_3d[points_3d[:, 2] > 0, :]
    ...
    pts1_back = np.matmul(P1, np.concatenate((points_3d.T, np.ones(shape=(1, points_3d.shape[0]))), axis=0))[:2, :].T
    pts2_back = np.matmul(P2, np.concatenate((points_3d.T, np.ones(shape=(1, points_3d.shape[0]))), axis=0))[:2, :].T
    pts1_back_denorm = cv2.undistortPoints(pts1_back, K1, None)
    pts2_back_denorm = cv2.undistortPoints(pts2_back, K2, None)

    for pts in pts1_back_denorm:
        cv2.circle(img1, (int(pts[1]), int(pts[0])), 10, (0, 255, 0), 3)
    for pts in pts1:
        cv2.circle(img1, (int(pts[1]), int(pts[0])), 10, (255, 0, 0), 3)
    cv2.imwrite("original_and_reproj1.png", img1)
    for pts in pts2_back_denorm:
        cv2.circle(img2, (int(pts[1]), int(pts[0])), 10, (0, 255, 0), 3)
    for pts in pts2:
        cv2.circle(img2, (int(pts[1]), int(pts[0])), 10, (255, 0, 0), 3)
    cv2.imwrite("original_and_reproj2.png", img2)
